...
<body>
    <form method="post" action="" id="mainForm" class="form-horizontal" role="form">
        {{ Form::token() }}
        <!-- navbar -->
    ...
    <table class="display table table-hover table-bordered table-striped AcademicGridView" cellspacing="0" align="Right" id="MainBodyContent_AcademicGridView" style="border-collapse:collapse;">
        ...
        <tr>
            ...
            <td>{{ $q->level_of_education }}</td>
            <td>{{ $q->exam_title }}</td>
            <td>{{ $q->major }} s</td>
            <td>Name of School
                <br/><strong>Name:</strong> {{ $q->insOther }}
            </td><td>
                <strong>Grade: </strong><br/>
                CGPA 5.00 (Out of Scale 5.00)
                <br/> <strong>Year of Passing: </strong>{{ $q->year_of_passing }}
            </td><td>{{ $q->duration }}</td><td>{{ $q->achievement }}</td><td>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            jQuery('.resultText').change(function () {
                ...
                if (selectValue == "Grade") {
                    $('.cgpaDiv').show("slow");
                    $('.marksDIv').hide("slow");
                } else if (selectValue == "First Division/Class" || selectValue == "Second Division/Class" || selectValue == "Third Division/Class") {
                    $('.marksDIv').show("slow");
                    $('.cgpaDiv').hide("slow");
                } else {
                    $('.cgpaDiv').hide("slow");
